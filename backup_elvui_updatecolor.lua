...
local function RestoreElvUIColoring(frame)
    ...
    Debug("Restored original coloring settings for frame")
end
...
local function UpdateHealthBarColor(health, unit)
    ...
    if unit == "target" or unit == "focus" then
        ...
        Debug("GUID for " .. unit .. ": " .. (guid or "nil"))
        
        if not guid then
            Debug("No GUID found for " .. unit)
            return
        end
        ...
        if UnitIsTapDenied(unit) then
            ...
            if frame then
                ...
                -- Apply gray color for tapped units
                health:SetStatusBarColor(0.5, 0.5, 0.5) -- Gray color
                Debug("Gray color set for tapped unit")
            else
